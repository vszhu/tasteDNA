...
`src/lib/group-sessions/medication-checks.ts` screens the shared menus. Review and warning dishes are excluded for that person before the existing fairness calculation. Ingredient gaps, recipe estimates, and unsupported medications stay in review. A candidate venue is withheld if it has no eligible dish for any participating member when nonempty medication lists are active. If none remain, computation retains the existing review-needed error and assigns no dish. The fix supplies ingredient evidence to the original workflow; it does not add a new review process. Timing/portion findings are held for review too; the app does not assume the user’s dosing schedule or treat these as permanent food bans.

The original campus import omitted ingredients. Its shared-menu reader now adds source-backed components where available and explicitly labeled recipe estimates elsewhere, preserving user-uploaded menus and account data. Estimates never become medication clearance. The same ingredient/source labels appear in candidate menus and assigned dishes. See [the ingredient repair handoff](team/GROUP_MENU_INGREDIENT_REPAIR.md) for source gates, coverage, snapshot invalidation, and verification.
