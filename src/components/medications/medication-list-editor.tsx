...
import { Check, LockKeyhole, Pill, Plus, Search, Trash2, X } from "lucide-react";
...
export function MedicationListEditor() {
  ...
  return (
      <section aria-labelledby="medication-list-title" className="rounded-[1.8rem] border border-[var(--line)] bg-white p-5 sm:p-7">
        ...
        <p className="mt-3 text-sm leading-6 text-[var(--muted)]">Choose the exact medicine and form on your label. This first version covers selected food interactions for {MEDICATION_CATALOG.length} medications.</p>
        ...
        <div className="mt-3 space-y-2">
          {matches.map((entry) => {
            ...
            return <button key={entry.id} type="button" disabled={!hydrated || full || selected} onClick={() => add(entry.id)} aria-label={`Add ${entry.name}, ${entry.form}`} className="flex w-full items-center justify-between gap-3 rounded-xl border border-[var(--line)] p-3.5 text-left transition-colors hover:bg-[var(--cream)] focus-visible:outline-2 focus-visible:outline-offset-2 disabled:opacity-55">
        ...
        <div className="mt-5 rounded-xl border border-[#d5dcd8] bg-[#f1f4f2] p-4"><p className="flex items-center gap-2 text-xs font-bold"><LockKeyhole className="size-3.5" /> Kept in this browser tab</p><p className="mt-2 text-xs leading-5 text-[var(--muted)]">Your list is stored in tab session storage, separate from taste ratings. It is not sent to AI, saved to your cloud account, or shared with dining groups. Clear it when using a shared device.</p></div>
      </section>
  );
}
