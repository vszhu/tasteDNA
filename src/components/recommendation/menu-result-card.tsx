...
export function MenuResultCard({ recommendation, feedback, currency, onFeedback }: {
  ...
  return (
    <article className={cn("overflow-hidden rounded-[1.4rem] border bg-white shadow-[0_10px_35px_rgba(45,37,31,.045)] transition-all duration-500", style.shell)}>
      <button type="button" className="w-full p-4 text-left sm:p-5" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded}>
        <div className="flex items-start gap-3 sm:gap-5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl leading-tight sm:text-2xl">{recommendation.dish.name}</h2>{recommendation.rank === 1 && <Badge className="text-[var(--tomato)]"><Heart className="mr-1 size-3 fill-current" /> TOP PICK</Badge>}</div><p className="mt-2 line-clamp-2 max-w-2xl text-sm leading-6 text-[var(--muted)]">{recommendation.dish.description}</p></div><div className={cn("grid size-16 shrink-0 place-items-center rounded-full sm:size-[4.5rem]", style.score)}><div className="text-center"><span className="display text-3xl leading-none">{recommendation.score}</span><span className="text-[10px] font-bold">%</span></div></div></div>
      ...
      <div className="flex items-center gap-2 border-t border-[var(--line)] px-4 py-3 sm:justify-end sm:px-5"><span className="mr-auto hidden text-xs text-[var(--muted)] sm:block">Help your TasteDNA learn</span><Button variant={feedback === 1 ? "default" : "ghost"} size="sm" aria-pressed={feedback === 1} onClick={() => onFeedback(false)}><ThumbsDown className={cn("size-3.5", feedback === 1 && "fill-current")} /> Not for me</Button><Button variant={feedback === 5 ? "accent" : "outline"} size="sm" aria-pressed={feedback === 5} onClick={() => onFeedback(true)}><ThumbsUp className={cn("size-3.5", feedback === 5 && "fill-current")} /> I’d order this</Button></div>
    </article>
  );
}
