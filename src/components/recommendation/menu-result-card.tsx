...
export function MenuResultCard({ recommendation, feedback, currency, onFeedback }: {
  ...
  return (
    <article className={cn("overflow-hidden rounded-[1.4rem] border bg-white shadow-[0_10px_35px_rgba(45,37,31,.045)] transition-all duration-500", style.shell)}>
      ...
      <div className="flex items-center gap-2 border-t border-[var(--line)] px-4 py-3 sm:justify-end sm:px-5"><span className="mr-auto hidden text-xs text-[var(--muted)] sm:block">Help your TasteDNA learn</span><Button variant={feedback === 1 ? "default" : "ghost"} size="sm" onClick={() => onFeedback(false)}><ThumbsDown className={cn("size-3.5", feedback === 1 && "fill-current")} /> Not for me</Button><Button variant={feedback === 5 ? "accent" : "outline"} size="sm" onClick={() => onFeedback(true)}><ThumbsUp className={cn("size-3.5", feedback === 5 && "fill-current")} /> I’d order this</Button></div>
    </article>
  );
}
